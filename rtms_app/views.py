...
def generate_calendar_weeks(patient):
    ...
    while current <= end_date:
        ...
        if current == patient.mapping_date or current in mapping_dates:
            day_info['events'].append({'type': 'mapping', 'label': '位置決め', 'url': build_url("mapping_add", args=[patient.id], query={"date": current.strftime("%Y-%m-%d")})})
            ...
            if session_num > 0 and session_num <= 30:
                ...
                if timing:
                    day_info['events'].append({
                        'type': 'assessment',
                        'label': label,
                        'url': build_url('assessment_add', [patient.id], {'date': current.strftime('%Y-%m-%d'), 'timing': timing})
                    })
        
        # 5. 退院
        if current == patient.discharge_date:
             day_info['events'].append({'type': 'discharge', 'label': '退院', 'url': build_url('patient_summary', [patient.id])})
...
def patient_clinical_path(request, patient_id):
    ...
    "label": "クリニカルパス（印刷）",
    "icon": "fa-print",
    "formaction": f"/app/patient/{patient.id}/path/print/",
    "formtarget": "_blank",
    }]
