...
def dashboard_view(request):
    ...
    for p in active_candidates:
        ...
        if not any(t['obj'] == p and t.get('is_discharge') == False for t in treatments):
            treatments.append({
...
def treatment_add(request, patient_id):
    ...
    if request.method == 'POST':
        ...
        if target_date_str:
            ...
            if t: initial_date = initial_date.replace(year=t.year, month=t.month, day=t.day)
...
def assessment_add(request, patient_id):
    ...
    hamd_items = [
        ...
        ('q3', '3. 自殺', 4, "0. なし<br>1. 生きる価値がないと感じる<br>2. 死ねたらという願望、または自己の死の可能性を考える<br>3. 自殺念慮、自殺をほのめかす行動をとる<br>4. 自殺を企図する"),
        ('q4', '4. 入眠障害', 2, "0. 入眠困難はない<br>1. 時々寝つけない、と訴える（すなわち、30分以上、週に2-3日）<br>2. 夜ごと寝つけない、と訴える（すなわち、30分以上、週に4日以上）"),
        ('q5', '5. 熟眠障害', 2, "0. 熟眠困難はない<br>1. 夜間、睡眠が不安定で、妨げられると訴える（または、時々、すなわち週に2-3日、夜中に30分以上覚醒している）<br>2. 夜中に目が覚めてしまう―トイレ以外で、寝床から出てしまういかなる場合も含む（しばしば、すなわち週に4日以上、夜中に30分以上覚醒している）"),
        ('q6', '6. 早朝睡眠障害', 2, "0. 早朝睡眠に困難はない<br>1. 早朝に目が覚めるが、再び寝つける（時々、すなわち、週に2～3日、早朝に30分以上目が覚める）<br>2. 一度起き出すと、再び寝つくことはできない（しばしば、すなわち、週に4日以上、早朝に30分以上目が覚める）"),
        ('q7', '7. 仕事と活動', 4, "0. 困難なくできる<br>1. 活動、仕事、あるいは趣味に関連して、それができない、疲れる、弱気であるといった思いがある（興味や喜びは軽度減退しているが、機能障害は明らかではない）<br>2. 活動・趣味・仕事に対する興味の喪失―患者が直接訴える、あるいは、気乗りのなさ、優柔不断、気迷いから間接的に判断される（仕事や活動をするのに無理せざるを得ないと感じる興味や喜び、機能は明らかに減退している）<br>3. 活動に費やす実時間の減少、あるいは生産性の低下（興味や喜び、機能の深刻な減退）<br>4. 現在の病気のために、働くことをやめた（病気のために仕事あるいは主要な役割を果たすことができない、そして興味も完全に喪失している）"),
        ('q8', '8. 精神運動抑制', 4, "0. 発話・思考は正常である<br>1. 面接時に軽度の遅滞が認められる（または、軽度の精神運動抑制）<br>2. 面接時に明らかな遅滞が認められる（すなわち、中等度、面接はいくらか困難；話は途切れがちで、思考速度は遅い）<br>3. 面接は困難である（重度の精神運動抑制、話はかなり長く途切れてしまい、面接は非常に困難）<br>4. 完全な昏迷（極めて重度の精神運動抑制：昏迷：面接はほとんど不可能）"),
        ('q9', '9. 精神運動激越', 4, "0. なし（正常範囲内の動作）<br>1. そわそわする<br>2. 手や髪などをいじくる<br>3. 動き回る、じっと座っていられない<br>4. 手を握りしめる、爪を噛む、髪を引っ張る、唇を噛む（面接は不可能）"),
        ('q10', '10. 不安, 精神症状', 4, "0. 問題なし<br>1. 主観的な緊張とイライラ感（軽度、一時的）<br>2. 些細な事柄について悩む（中等度、多少の苦痛をもたらす、あるいは実在する問題に過度に悩んでいる）<br>3. 心配な態度が顔つきや話し方から明らかである（重度：不安のために機能障害が生じている）<br>4. 疑問の余地なく恐怖が表出されている（何もできない程の症状）"),
        ('q11', '11. 不安, 身体症状', 4, "0. なし<br>1. 軽度（症状は時々出現するのみ、機能の障害はない。わずかな苦痛）<br>2. 中等度（症状はより持続する、普段の活動に多少の支障をきたす、中等度の苦痛）<br>3. 重度（顕著な機能の障害）<br>4. 何もできなくなる"),
        ('q12', '12. 身体症状, 消化器系', 2, "0. なし<br>1. 食欲はないが、促されなくても食べている（普段より食欲はいくらか低下）<br>2. 促されないと食事摂取が困難（あるいは、無理して食べなければならないかどうかに関わらず、食欲は顕著に低下している）"),
        ('q13', '13. 身体症状, 一般的', 2, "0. なし<br>1. 手足や背中、あるいは頭の重苦しさ。背部痛、頭痛、筋肉痛。元気のなさや易疲労性（普段より気力はいくらか低下：軽度で一時的な、気力の喪失や筋肉の痛み／重苦しさ）<br>2. 何らかの明白な症状（持続的で顕著な、気力の喪失や筋肉の痛み／重苦しさ）"),
        ('q14', '14. 生殖器症状', 2, "0. なし<br>1. 軽度（普段よりいくらか関心が低下）<br>2. 重度（普段よりかなり関心が低下）"),
        ('q15', '15. 心気症', 4, "0. なし（不適切な心配はない、あるいは完全に安心できる）<br>1. 体のことが気がかりである（自分の健康に関する多少の不適切な心配、または大丈夫だと言われているにも関わらず、わずかに心配している）<br>2. 健康にこだわっている（しばしば自身の健康に対し過剰に心配する、あるいは医学的に大丈夫だと明言されているにも関わらず、特別な病気があると思い込んでいる）<br>3. 訴えや助けを求めること等が頻繁にみられる（医師が確認できていない身体的問題があると確信している：身体的な健康についての誇張された、現実的でない心配）<br>4. 心気妄想（例えば、体の一部が衰え、腐ってしまうと感じる、など、外来患者ではまれである）"),
        ('q16', '16. 体重減少', 2, "0. 体重減少なし、あるいは今回の病気による減少ではない<br>1. 今回のうつ病により、おそらく体重が減少している<br>2. （患者によると）うつ病により、明らかに体重が減少している"),
        ('q17', '17. 病識', 2, "0. うつ状態であり病気であることを認める、または現在うつ状態でない<br>1. 病気であることを認めるが、原因を粗食、働き過ぎ、ウィルス、休息の必要性などのせいにする（病気を否定するが、病気である可能性は認める）<br>2. 病気であることを全く認めない（病気であることを完全に否定する）"),
        ('q18', '18. 日内変動', 2, "<strong>A. 変動の有無・方向</strong><br>0: なし, 1: 午前, 2: 午後<br><br><strong>B. 変動の程度 (※こちらをスコアとして入力)</strong><br>0. なし<br>1. 軽度<br>2. 重度"),
        ('q19', '19. 現実感喪失, 離人症', 4, "0. なし<br>1. 軽度<br>2. 中等度<br>3. 重度<br>4. 何もできなくなる"),
