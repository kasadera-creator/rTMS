...
def get_session_number(start_date, target_date):
    """開始日からターゲット日までの平日（月-金）の日数を計算して返す"""
...
def dashboard_view(request):
    ...
    for p in active_candidates:
        ...
        if session_count_so_far >= 30: pass # 完了済み
...
def treatment_add(request, patient_id):
    ...
    return render(request, 'rtms_app/treatment_add.html', {
        ...
        'session_num': session_num, 'week_num': week_num, 'end_date_est': end_date_est, 'start_date': patient.first_treatment_date, 
        'dashboard_date': dashboard_date, 'alert_msg': alert_msg, 'instruction_msg': instruction_msg, 'judgment_info': judgment_info
    })
