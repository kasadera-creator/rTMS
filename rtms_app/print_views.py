...
def _build_discharge_context(request, patient_id):
	...
	context = {
		...
		'diagnosis': treatment_course.diagnosis if treatment_course else patient.diagnosis,
		'summary_text': summary_text,
		'discharge_prescription': discharge_prescription,
		'attending_physician': treatment_course.attending_physician if treatment_course else patient.attending_physician,
		'today': timezone.now().date(),
...
def _build_referral_context(request, patient_id):
	...
	context = {
		...
		'discharge_date': discharge_date,
		'diagnosis': treatment_course.diagnosis if treatment_course else patient.diagnosis,
		'referral_source': treatment_course.referral_source if treatment_course else patient.referral_source,
		'referral_doctor': treatment_course.referral_doctor if treatment_course else patient.referral_doctor,
		'summary_text': summary_text,
		'attending_physician': treatment_course.attending_physician if treatment_course else patient.attending_physician,
		'today': timezone.now().date(),
		'test_scores': test_scores,
